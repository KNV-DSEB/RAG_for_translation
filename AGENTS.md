...
- STT: **faster-whisper `small`, lượng tử int8, chạy CPU-only**.
  Máy có GTX 1650 nhưng driver 462.62 quá cũ so với CUDA của torch hiện nay → không dùng GPU.
...
- Không có ffmpeg trên PATH → giải mã audio bằng PyAV (`av`), không cần cài hệ thống
...
- `/web/js/audio.js` — ghi âm bằng MediaRecorder + thước đo mức âm (thay `st.audio_input`)
- `/data` — SQLite DB, ChromaDB, `documents/`, `recordings/`, `tts_cache/`, `test_documents/`
- `/_specs` — spec files cho từng feature (do Codex tạo trong Plan Mode)
...
- Error handling: luôn try/except cho gọi API bên ngoài (Gemini, Whisper, TTS)
...
- Test mock dịch end-to-end: audio input → STT → LLM scoring → hiển thị kết quả, 3 buổi liên tiếp
...
## Đồng bộ tài liệu
Bất cứ thay đổi nào xung khắc với AGENTS.md phải cập nhật AGENTS.md **trong cùng lần thay đổi đó**,
không tích lại sửa sau. AGENTS.md luôn phải là mô tả đúng của hệ thống thật.
